...
@Component
public class CompareGitHubInfoStrategy
    implements CompareInfoStrategy<GitHubInfo, GitHubResponse, ResultOfCompareGitHubInfo> {
    @Override
    public ResultOfCompareGitHubInfo compare(GitHubInfo savedInfo, GitHubResponse loadedResponse) {
        ...
        Optional<OffsetDateTime> lastActiveTime = Optional.ofNullable(
                (savedInfo.getLastActiveTime().equals(loadedResponse.getInfoResponse().getUpdatedAt()) ? null
                    : loadedResponse.getInfoResponse().getUpdatedAt())
        );
