...
public class JPACheckUpdateLinksDAServiceImpl implements CheckUpdateLinksDAService {
    ...
    public List<WebsiteInfo> getListWebsiteInfoByLastCheckTime(int count) {
        List<WebsiteInfoEntity> loadedEntities = websiteInfoDAO.loadWebsiteInfoWithTheEarliestUpdateTime(count);
        return loadedEntities.stream().map(WebsiteInfoEntity::getWebsiteInfo).toList();
    }
}
