...
class HandlerUpdateGitHubInfoChainTest {
    ...
    void handleNotUpdatedLinkTest() {
        ...
        GitHubCommitResponse[] commitResponses = Stream.of(
                new GitHubCommitResponse(
                    "12345",
                    new GitHubNestedCommitResponse(new GitHubCommiterResponse(OffsetDateTime.now()))
                ),
                new GitHubCommitResponse(
                    "67890",
                    new GitHubNestedCommitResponse(new GitHubCommiterResponse(OffsetDateTime.now()))
        ...
        //Assert
        assertFalse(
            optionalResultFromSUT.isPresent(),
            () -> "LinkUpdateRequest must be null.\n Description of update is "
    ...
    void handleUpdatedLinkTest(
        GitHubResponse responseForReturnFromWebClient,
        ...
        assertTrue(
            Arrays.equals(expectedResultRequest.getTgChatIds(), resultFromSUT.getTgChatIds()),
        ...
        assertTrue(
            compareResultFromSUT.isDifferent(),
        ...
        public Stream<? extends Arguments> provideArguments(ExtensionContext context) {
            Arguments firstArguments;
            {
                ...
                GitHubBranchResponse[] branchResponses = Stream.of(
                    ...
                    new GitHubBranchResponse("hw5")
                ).toArray(GitHubBranchResponse[]::new);
                GitHubCommitResponse[] commitResponses = Stream.of(
                        new GitHubCommitResponse(
                            "qwert",
                            new GitHubNestedCommitResponse(new GitHubCommiterResponse(lastCheckUpdateDate))
                        ),
                        new GitHubCommitResponse(
                            "67890",
                            new GitHubNestedCommitResponse(new GitHubCommiterResponse(lastCheckUpdateDate))
                ...
                LinkUpdateRequest expectedResultRequest = new GitHubBuilderLinkUpdateRequest().buildLinkUpdateRequest(
                    expectedResultOfCompare,
                    chatIdsForFirstTest
                );
